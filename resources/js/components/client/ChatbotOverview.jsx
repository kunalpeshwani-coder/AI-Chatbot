...
export default function ChatbotOverview({ chatbot, onUpdate }) {
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden min-h-0">
            ...
            {/* Header — transparent, sits on body colour */}
            <div className="flex-shrink-0 px-2 pb-5">
                ...
                {chatbot.description && (
                    <p className="text-navy-300 text-sm">{chatbot.description}</p>
                )}
            </div>

            {/* Card — tabs + content */}
            <div className="flex-1 flex flex-col bg-navy-950 rounded-2xl border border-white/10 overflow-hidden min-h-0">

                {/* Tab bar */}
                <div className="flex border-b border-white/10 flex-shrink-0 px-2">
                    {TABS.map(t => (
                        <button
                            key={t.key}
